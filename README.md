...
- 递归思路：isMatch(ssss, x\*xxx)=isMatch(ssss, xxx) | | ((s==x||x=='.')&&isMatch(sss, x\*xxx))；以及isMatch(ssss, xxxx)=isMatch(sss, xxx)；得到状态转移方程，通过该方程递推判断；其实这是一个深度搜索，对所有情况递归遍历。时间复杂度，可以对最坏情况进行分析，即正则式是这种形式'x\*x\*x\*'，则每次递归都会进入两个子程序，考虑近似于遍历满二叉树，则树的最大深度为T+P/2，则时间复杂度O($2^{T+P/2}$)；达到指数级
- 动态规划思路：
